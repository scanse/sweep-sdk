...
device_s device_construct(const char* port, int32_t bitrate) {
  ...
  return new device{h_comm, os_reader, FALSE, 500};
}
...
void device_read(device_s serial, void* to, int32_t len) {
  ...
  if (serial->waiting_on_read) {
    auto dwRes = WaitForSingleObject(serial->os_reader.hEvent, serial->read_timeout_millis);
    switch (dwRes) {
    // Read completed.
    case WAIT_OBJECT_0:
  ...
  if (dw_num_read != (DWORD)len) {
    SWEEP_ASSERT(false && "reliable read failed to read requested number of bytes");
    throw error{"reading from serial device failed"};
  }
}
